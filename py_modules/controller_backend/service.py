...
from controller_backend.daemon.commands import CliRunner, run_recover, run_status
...
class Service:
    def __init__(self, *, emit: EmitCallback, plugin_dir: str, settings_dir: str, runtime_dir: str, log_dir: str,
        ...
        self.drd_known_enabled = False   # modprobe-assisted DRD probe, done once at startup
    ...
    async def probe_drd(self) -> None:
        """The modprobe-assisted DRD check (a subprocess) once; the status loop only reads the driver link."""
        try:
            drd = await asyncio.to_thread(detect_drd, self.sysfs_root, True)
        except Exception:
            self.log.exception("DRD probe failed")
            return
        self.drd_known_enabled = drd.enabled
        self.log.info("DRD %s (via %s)", "enabled" if drd.enabled else "not detected", drd.via)

    def build_status(self) -> JsonDict:
        return build_status(
            plugin_version=self.plugin_version,
            facts=status_facts(self.sysfs_root, self.dev_root, self.drd_known_enabled),
    ...
    async def emit_status(self) -> None:
        try:
            await self._emit("status", self.build_status())
        except Exception:
            self.log.exception("emit status failed")

    async def diagnostics(self) -> JsonDict:
        cli_status, cli_error = await run_status(self.cli)   # the daemon's own view, for comparison
        return build_diagnostics(
            ...
            cli_status_raw=cli_status, cli_status_error=cli_error, last_recover=self.last_recover,
        ...
        while True:
            try:
                if self.supervisor.alive():
                    ...
                    signature = connectivity_signature(status_facts(self.sysfs_root, self.dev_root, self.drd_known_enabled))
